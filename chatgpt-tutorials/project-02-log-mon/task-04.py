...
# This code tracks all errors together, not per message
...
def duplicate_check(file_name):
       ...
       with open(file_name, 'r') as file:
        ...
        for line in file:
            ...
            recent_error_times.append(log_time)
            ...
            while recent_error_times and (log_time - recent_error_times[0]) > time_window:
                recent_error_times.popleft()

            # Check if the number of error occurrences 
            # within the time window exceeds the threshold.
            if len(recent_error_times) >= threshold: 
                alert_message = (
                    f"THRESHOLD ALERT: {len(recent_error_times)} errors occurred within 10 seconds "
                    f"at {log_time.strftime('%Y-%m-%d %H:%M:%S')}"
                )
                print(alert_message)
